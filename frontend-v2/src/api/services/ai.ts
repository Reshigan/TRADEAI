...
export const aiService = {
  ...
  getInsights: async (context?: string): Promise<AIInsight[]> => {
    const response = await apiClient.get('/ai/insights', { params: { context } });
    return response.data;
  },

  // Get AI recommendations
  getRecommendations: async (type?: string): Promise<AIRecommendation[]> => {
    const response = await apiClient.get('/ai/recommendations', { params: { type } });
    return response.data;
  },

  // Get promotion recommendations
  getPromotionRecommendations: async (customerId?: string, productId?: string) => {
    const response = await apiClient.post('/ai/promotion/recommendations', {
      customerId,
      productId,
    });
    return response.data;
  ...
  getCustomerIntelligence: async (customerId: string) => {
    const response = await apiClient.get(`/ai/customers/${customerId}/intelligence`);
    return response.data;
  },

  // Price optimization
  optimizePrice: async (productId: string, context: any) => {
    const response = await apiClient.post(`/ai/products/${productId}/optimize-price`, context);
    return response.data;
  },

  // Predict demand
  predictDemand: async (productId: string, dateRange: any): Promise<PredictionResult> => {
    const response = await apiClient.post(`/ai/products/${productId}/predict-demand`, dateRange);
    return response.data;
  },

  // Analyze promotion effectiveness
  analyzePromotion: async (promotionId: string) => {
    const response = await apiClient.get(`/ai/promotions/${promotionId}/analyze`);
    return response.data;
  },

  // Get smart suggestions for form fields
  getSuggestions: async (field: string, context: any) => {
    const response = await apiClient.post('/ai/suggestions', { field, context });
    return response.data;
  },

  // Validate data with AI
  validateData: async (data: any, type: string) => {
    const response = await apiClient.post('/ai/validate', { data, type });
    return response.data;
  },
