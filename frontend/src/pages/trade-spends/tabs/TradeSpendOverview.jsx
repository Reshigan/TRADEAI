      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <Typography variant="body1">{tradeSpend.customer?.name || 'N/A'}</Typography>
          </Grid>
          <Grid item xs={12} md={6}>
            <Typography variant="body2" color="text.secondary">Vendor</Typography>
            <Typography variant="body1">{tradeSpend.vendor?.name || 'N/A'}</Typography>
          </Grid>
        </Grid>
      </Paper>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={3}>
            ...
            <Typography variant="h6">R {tradeSpend.amount?.requested?.toLocaleString() || '0'}</Typography>
          </Grid>
          <Grid item xs={12} md={3}>
            <Typography variant="body2" color="text.secondary">Approved</Typography>
            <Typography variant="h6">R {tradeSpend.amount?.approved?.toLocaleString() || '0'}</Typography>
          </Grid>
          <Grid item xs={12} md={3}>
            <Typography variant="body2" color="text.secondary">Spent</Typography>
            <Typography variant="h6">R {tradeSpend.amount?.spent?.toLocaleString() || '0'}</Typography>
          </Grid>
          <Grid item xs={12} md={3}>
            <Typography variant="body2" color="text.secondary">Remaining</Typography>
            <Typography variant="h6">
              R {((tradeSpend.amount?.approved || 0) - (tradeSpend.amount?.spent || 0)).toLocaleString()}
            </Typography>
          </Grid>
        </Grid>
      </Paper>
