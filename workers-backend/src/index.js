...
import { seedRoutes, seedAuthRoutes } from './routes/seed.js';
...
app.use('*', cors({
  origin: (origin) => {
    const defaults = ['https://tradeai.vantax.co.za', 'http://localhost:3000', 'http://localhost:5173', 'http://localhost:12000'];
...
app.route('/api/auth/seed', seedAuthRoutes);
