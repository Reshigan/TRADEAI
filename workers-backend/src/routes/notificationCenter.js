...
notif.post('/notifications', async (c) => {
  try {
    ...
    const created = await db.prepare('SELECT * FROM notifications WHERE id = ? AND company_id = ?').bind(id, companyId).first();
...
notif.put('/notifications/:id/read', async (c) => {
  try {
    ...
    const now = new Date().toISOString();
    await db.prepare("UPDATE notifications SET status = 'read', read_at = ? WHERE id = ? AND company_id = ?").bind(now, id, companyId).run();
...
notif.put('/notifications/:id/dismiss', async (c) => {
  try {
    ...
    const { id } = c.req.param();
    const now = new Date().toISOString();
    await db.prepare("UPDATE notifications SET status = 'dismissed', dismissed_at = ? WHERE id = ? AND company_id = ?").bind(now, id, companyId).run();
...
notif.delete('/notifications/:id', async (c) => {
  try {
    const db = c.env.DB;
    const { id } = c.req.param();
    await db.prepare('DELETE FROM notifications WHERE id = ? AND company_id = ?').bind(id, companyId).run();
...
notif.post('/rules', async (c) => {
  try {
    ...
    const created = await db.prepare('SELECT * FROM alert_rules WHERE id = ? AND company_id = ?').bind(id, companyId).first();
    return c.json({ success: true, data: created }, 201);
  } catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
...
notif.put('/rules/:id', async (c) => {
  try {
    ...
    await db.prepare(`UPDATE alert_rules SET name = ?, description = ?, rule_type = ?, category = ?, entity_type = ?, metric = ?, operator = ?, threshold_value = ?, threshold_unit = ?, severity = ?, is_active = ?, frequency = ?, cooldown_minutes = ?, notes = ?, data = ?, updated_at = ? WHERE id = ? AND company_id = ?`).bind(body.name || existing.name, body.description ?? existing.description, body.rule_type || existing.rule_type, body.category || existing.category, body.entity_type ?? existing.entity_type, body.metric ?? existing.metric, body.operator || existing.operator, body.threshold_value ?? existing.threshold_value, body.threshold_unit ?? existing.threshold_unit, body.severity || existing.severity, body.is_active !== undefined ? (body.is_active ? 1 : 0) : existing.is_active, body.frequency || existing.frequency, body.cooldown_minutes ?? existing.cooldown_minutes, body.notes ?? existing.notes, JSON.stringify(body.data || JSON.parse(existing.data || '{}')), now, id).run();
    const updated = await db.prepare('SELECT * FROM alert_rules WHERE id = ? AND company_id = ?').bind(id, companyId).first();
...
notif.delete('/rules/:id', async (c) => {
  try {
    const db = c.env.DB;
    const { id } = c.req.param();
    await db.prepare('DELETE FROM alert_rules WHERE id = ? AND company_id = ?').bind(id, companyId).run();
...
notif.post('/history', async (c) => {
  try {
    ...
    const created = await db.prepare('SELECT * FROM alert_history WHERE id = ? AND company_id = ?').bind(id, companyId).first();
    return c.json({ success: true, data: created }, 201);
  } catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
...
notif.put('/history/:id/acknowledge', async (c) => {
  try {
    ...
    const { id } = c.req.param();
    const now = new Date().toISOString();
    await db.prepare("UPDATE alert_history SET status = 'acknowledged', acknowledged_by = ?, acknowledged_at = ? WHERE id = ? AND company_id = ?").bind(getUserId(c), now, id).run();
...
notif.put('/history/:id/resolve', async (c) => {
  try {
    ...
    const { id } = c.req.param();
    const now = new Date().toISOString();
    await db.prepare("UPDATE alert_history SET status = 'resolved', resolved_by = ?, resolved_at = ? WHERE id = ? AND company_id = ?").bind(getUserId(c), now, id).run();
...
notif.delete('/history/:id', async (c) => {
  try {
    const db = c.env.DB;
    const { id } = c.req.param();
    await db.prepare('DELETE FROM alert_history WHERE id = ? AND company_id = ?').bind(id, companyId).run();
    return c.json({ success: true, message: 'Deleted' });
  } catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
