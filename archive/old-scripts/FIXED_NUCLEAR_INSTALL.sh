...
# 💥 TRADEAI FIXED NUCLEAR CLEAN & HOME INSTALL
# Complete system wipe and fresh home directory installation
# FIXED: Correct domain and PM2 configuration
# Domain: tradeai.gonxt.tech
# Email: reshigan@gonxt.tech
# Author: OpenHands AI Assistant
# Version: FIXED-NUCLEAR-1.0
...
BACKUP_DIR="$HOME_DIR/backups/fixed-nuclear-backup-$(date +%Y%m%d-%H%M%S)"
...
show_warning() {
    echo ""
    echo -e "${BOLD}${RED}⚠️  FIXED NUCLEAR CLEAN & INSTALL WARNING ⚠️${NC}"
    echo -e "${BOLD}${RED}===========================================${NC}"
    echo ""
    ...
    echo ""
    echo -e "${GREEN}Then install fresh with CORRECT configuration:${NC}"
    echo -e "${GREEN}• Domain: $DOMAIN${NC}"
    echo -e "${GREEN}• Email: $EMAIL${NC}"
    echo -e "${GREEN}• Install Directory: $APP_DIR${NC}"
    echo -e "${GREEN}• FIXED PM2 configuration${NC}"
    echo ""
    echo -e "${YELLOW}Backup will be created at: $BACKUP_DIR${NC}"
    echo ""
    read -p "Are you ABSOLUTELY SURE you want to proceed? (type 'FIXED' to confirm): " confirm
    
    if [ "$confirm" != "FIXED" ]; then
        echo "Operation cancelled."
        exit 0
    fi
    
    echo ""
    echo -e "${BOLD}${RED}🚨 FIXED NUCLEAR CLEAN INITIATED 🚨${NC}"
    echo ""
    sleep 3
}
...
# FIXED backend setup with correct PM2 configuration
fresh_backend_setup() {
    step "FRESH BACKEND SETUP - FIXED PM2 Configuration"
    ...
    # Create FIXED PM2 config with correct app name
    fresh "Creating FIXED backend PM2 configuration..."
    sudo -u $CURRENT_USER cat > ecosystem.config.js << EOF
module.exports = {
  apps: [{
...
EOF
    ...
    log "✅ FIXED backend setup completed with correct PM2 configuration"
}

# FIXED frontend setup
fresh_frontend_setup() {
    step "FRESH FRONTEND SETUP - FIXED Configuration"
...
EOF
    ...
    # Create FIXED PM2 config with correct app name
    fresh "Creating FIXED frontend PM2 configuration..."
    sudo -u $CURRENT_USER cat > ecosystem.config.js << EOF
module.exports = {
  apps: [{
...
EOF
    ...
    log "✅ FIXED frontend setup completed with correct PM2 configuration"
}

# FIXED nginx configuration with correct domain
fresh_nginx_config() {
    step "FRESH NGINX CONFIGURATION - FIXED Domain"
    
    # Create FIXED nginx config with correct domain
    fresh "Creating FIXED nginx configuration for $DOMAIN..."
    cat > /etc/nginx/sites-available/tradeai << EOF
# TRADEAI FIXED Fresh Home Deployment Configuration
# Domain: $DOMAIN
server {
    listen 80;
...
EOF
    ...
    # Test configuration
    if nginx -t; then
        log "✅ FIXED nginx configuration is valid for $DOMAIN"
    else
    ...
    log "✅ FIXED nginx configuration completed for $DOMAIN"
}

# FIXED SSL setup with correct domain
fresh_ssl_setup() {
    step "FRESH SSL SETUP - FIXED Domain"
    ...
    fresh "Checking domain resolution for $DOMAIN..."
    ...
    # Request FIXED SSL certificate for correct domain
    fresh "Requesting FIXED SSL certificate for $DOMAIN..."
    if certbot --nginx \
        --non-interactive \
        --agree-tos \
        --email "$EMAIL" \
        ...
        log "✅ FIXED SSL certificate obtained for $DOMAIN"
...
# FIXED service startup with correct PM2 configuration
start_fresh_services() {
    step "STARTING FRESH SERVICES - FIXED PM2 Configuration"
    
    # Ensure proper ownership
    chown -R $CURRENT_USER:$CURRENT_USER "$APP_DIR"
    
    # Start FIXED backend with correct configuration
    fresh "Starting FIXED backend service..."
    cd "$APP_DIR/backend"
    ...
    log "✅ FIXED backend service started with correct name: tradeai-backend"
    
    # Start FIXED frontend with correct configuration
    fresh "Starting FIXED frontend service..."
    cd "$APP_DIR/frontend"
    sudo -u $CURRENT_USER pm2 start ecosystem.config.js
    ...
    log "✅ FIXED frontend service started with correct name: tradeai-frontend"
    
    # Save PM2 configuration
    sudo -u $CURRENT_USER pm2 save
    
    # Setup PM2 startup
    fresh "Setting up FIXED PM2 startup..."
    ...
    log "✅ All FIXED services started successfully"
}

# Comprehensive verification
verify_fixed_installation() {
    step "VERIFYING FIXED INSTALLATION"
    ...
    # Check FIXED PM2 processes
    fresh "Checking FIXED PM2 processes..."
    ...
    if [ -n "$backend_online" ]; then
        log "✅ FIXED Backend service (tradeai-backend) is online"
    else
        error "❌ FIXED Backend service (tradeai-backend) is not online"
        return 1
    fi
    
    if [ -n "$frontend_online" ]; then
        log "✅ FIXED Frontend service (tradeai-frontend) is online"
    else
        error "❌ FIXED Frontend service (tradeai-frontend) is not online"
        return 1
    fi
    ...
    if [ -d "/etc/letsencrypt/live/$DOMAIN" ]; then
        log "✅ SSL certificate is installed for $DOMAIN"
    else
    ...
    log "✅ FIXED installation verification completed"
}

# Show final results
show_final_results() {
    echo ""
    echo -e "${BOLD}${GREEN}💥 FIXED NUCLEAR CLEAN & FRESH INSTALL COMPLETED! 💥${NC}"
    ...
    echo ""
    echo -e "${BOLD}🏠 FIXED FRESH HOME INSTALLATION:${NC}"
    ...
    echo "   • Backend (tradeai-backend): $(sudo -u $CURRENT_USER pm2 list | grep -c "tradeai-backend.*online") processes"
    echo "   • Frontend (tradeai-frontend): $(sudo -u $CURRENT_USER pm2 list | grep -c "tradeai-frontend.*online") processes"
    echo ""
    ...
    echo ""
    echo -e "${BOLD}✅ FIXED CONFIGURATION BENEFITS:${NC}"
    echo "   🎯 Correct domain: $DOMAIN"
    echo "   📧 Correct email: $EMAIL"
    echo "   🏷️ Correct PM2 app names (tradeai-backend, tradeai-frontend)"
    echo "   🔧 Proper ecosystem.config.js files"
    echo "   🏠 Clean home directory installation"
    echo "   🔒 Zero permission issues"
    echo ""
    echo -e "${BOLD}${GREEN}🎉 TRADEAI is now running with FIXED configuration! 🎉${NC}"
}

# Main execution
main() {
    echo ""
    echo -e "${BOLD}${RED}💥 TRADEAI FIXED NUCLEAR CLEAN & FRESH INSTALL 💥${NC}"
    echo -e "${BOLD}${RED}=================================================${NC}"
    echo ""
    log "🚀 Starting FIXED Nuclear Clean & Fresh Install"
    log "FIXED Domain: $DOMAIN"
    log "FIXED Email: $EMAIL"
    ...
    log "🎉 FIXED Nuclear Clean & Fresh Install Completed Successfully!"
}
